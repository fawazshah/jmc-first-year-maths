...
\\
Let $ S \subset V $ be a spanning set, and let $ \vec{v}_{1}...\vec{v}_{n} \in V $ be a linearly independent subset. There always exists some $ \vec{s}_{1}...\vec{s}_{n} \in S $ such that
\begin{equation}
(S \setminus \{\vec{s}_{1}...\vec{s}_{n}\}) \cup \{\vec{v}_{1}...\vec{v}_{n}\}
\end{equation}
is still a spanning set. In other words, we can substitute in any linearly independent set, and $ S $ will still be a spanning set.
\\\\
Any linearly independent set is smaller than or equal to any spanning set.
\\\\
...
\\\\
If $ f : \R^{n} \to \R^{m} $ then $ f \equiv T_{A} $ for some matrix $ A  \in \textrm{Mat}_{n \times m}(\R) $.
\\\\
...
\begin{itemize}
\item $ \textrm{image}(f) = V $
\item $ \textrm{kernel}(f) = \{\vec{0}_{U}\} $
\end{itemize}
If $ U \simeq V $ then $ V \simeq U $.
\\\\
