...
\item $ \star $ has an identity element in $ G $
...
\\\\
The normal exponent rules apply.
\paragraph{Definition of $ [a] $ and $ \Z_{n} $}
For any $ a \in \Z $:
\begin{equation}
[a] = \{b \in \Z \ | \ b \equiv a \textrm{ mod } n \}
\end{equation}
Note that $ [a] $ forms an equivalence class, and there are exactly $ n $ of these equivalence classes. $ \Z_{n} $ is the set of all these equivalence classes.
\begin{equation}
\Z_{n} = \{[0], [1] ... [n - 1]\}
\end{equation}
\paragraph{Definition of $ \Z_{n}^{*} $}
$ \Z_{n}^{*} $ is the set of all $ [a] $ such that $ [a] $ is invertible. Note in this case the identity element is $ [1] $.
\begin{equation}
\Z_{n}^{*} = \{[a] \in \Z_{n} \ | \ \exists [b] \in Z_{n} \quad s.t. \quad [a] \star [b] = [1]\}
\end{equation}
Some common choices for $ \star $ are $ [a] + [b] = [a + b] $ and $ [a] \times [b] = [ab] $.
...
\paragraph{Cycle}
A permutation $ \sigma \in S_{n} $ is a cycle if there exists some $ a_{1}...a{k}  \in \{1...n\} $ such that:
\begin{equation}
