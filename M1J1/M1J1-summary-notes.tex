...
\item If the eigenvalues have the SAME sign, we can take the limit of the entire system as $ t \to \infty $ or as $ t \to - \infty $ to find which eigenvector 'dominates'. All contour lines will eventually tend to this eigenvector, even if they originated from the other one.
\\\\
If the sign of the eigenvalues is $ +ve $, we take $ t \to \infty $.
\\
If the sign is $ -ve $, we take $ t \to - \infty $.
\\\\
In both cases, the eigenvalue with the largest magnitude dominates.
\end{enumerate}
