...
To draw a phase portrait, we can follow some algorithmic steps:
\begin{enumerate}
...
\item Sketch out other contour lines on the x-y plane with arrows following the pattern of the existing lines
\end{enumerate}
...
\begin{enumerate}
\item Find the stationary points (where $ \frac{\partial z}{\partial x} = \frac{\partial z}{\partial y} = 0 $)
...
All scalar fields have an associated vector quantity called grad.
\\
All vector fields have an associated scalar quantity called div, and an associated vector quantity called curl.
\\
The definitions of these quantities are given in the definitions section.
...
If $ \vec{A} $ is an $ m \times n $ matrix, and $ \vec{e_{k}} $ is the $ k^{th} $ standard basis vector of $ \R^{m} $ (note $ m $ not $ n $), then $ \vec{A e_{k}} $ gives the column vector which is the $ k^{th} $ column of $ A $.
\\\\
...
\\
The process of Gaussian elimination can be used to reduce a matrix to REF or RREF. It is applied to augmented matrices to solve their corresponding systems of equations. Let the augmented matrix be $ (\vec{A} \ | \ \vec{b}) = (a_{ij} \ | \ b_{i}) $:
\begin{enumerate}
...
\end{equation}
Note the alternating signs present in the above equation. In general, for any $ n \times n $ matrix $ \vec{A} $ we have:
\begin{equation}
\det \vec{A} = \sum_{j=1}^{n} (-1)^{i+j} \det (\vec{A}_{ij})
\end{equation}
by expanding about any row $ i $.
\\\\
\textit{Properties of the determinant}
\\
...
\end{pmatrix}
\end{equation}
For an invertible $ n \times n $ matrix $ \vec{A} $, calculating its inverse can be done with the following algorithm:
\begin{enumerate}
...
\item The right matrix (which started as $ \vec{I}_{n} $) is now $ \vec{A}^{-1} $
\end{enumerate}
